          ...
          <For each={props.standings}>
            {(standing, index) => (
              <tr class={`border-t border-surface-lighter ${index() < props.advancingCount ? 'bg-primary/5' : ''}`}>
                ...
                <td class="px-2 py-2 font-semibold text-on-surface max-w-[8rem] truncate" title={props.teamNames[standing.teamId] ?? standing.teamId}>{props.teamNames[standing.teamId] ?? standing.teamId}</td>
